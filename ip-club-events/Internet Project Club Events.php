...
function ip_club_events($post_object) {
  ...
  ?>
    ...
    jQuery(document).ready(function($) {
		var clubId = 1;
		var url = "http://35.164.255.95:8081/api/ClubsWS/" + clubId + "/Events";		
		//var url = "http://thing120170201021741.azurewebsites.net/api/ClubsWS/" + clubId + "/Events";
        ...
        .done(function( data ) {
		  ...
		  for (var i = 0; i < data.length; i++) {
			...
			var openTo;
			if (item.isPublic) {
				console.log('check public: ' + item.isPublic);
				openTo = 'Everyone';
			}
			else {
				openTo = item.clubs;
